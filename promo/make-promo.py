...
把两张 Cursor 截图（汉化前 / 汉化后）做成宣传素材：
  ...
  2. 01-before-after.png   左右对比图（GitHub README / Social preview，1600x800）
...
换更高分辩率的截图时，把 REGIONS 里的坐标按比例改一下即可（当前按 1024x711 标定）。
...
def redact(path, body_boxes):
    ...
    for y0, y1 in SIDEBAR_ROWS:
        mosaic(img, S((SIDEBAR_X[0], y0, SIDEBAR_X[1], y1)), block=int(7 * sx))
    mosaic(img, S(USER_BOX), block=int(8 * sx))
    for b in body_boxes:
...
def framed(img, label, label_fill, fg=WHITE):
    ...
    d.text((W - 18 - d.textlength("cursor-zh · GitHub 搜 cursor-zh", font=font(17)), H + 20), "cursor-zh · GitHub 搜 cursor-zh", font=font(17), fill=GRAY)
...
def make_toggle():
    ...
    fps = 12
    seq = []  # (frame, duration_ms)
    seq += [(fb, 1800)]
    seq += [(f, 1000 // fps) for f in crossfade(fb, fa, 8)]
    seq += [(fa, 2600)]
    seq += [(f, 1000 // fps) for f in crossfade(fa, fb, 8)]
    frames = [f for f, _ in seq]
    durs = [dm for _, dm in seq]
    gif_frames = [f.convert("P", palette=Image.ADAPTIVE, colors=256) for f in frames]
    gif_frames[0].save(
        os.path.join(OUT, "02-toggle.gif"), save_all=True, append_images=gif_frames[1:], duration=durs, loop=0, optimize=False
    )
    # MP4：按时长展开为恒定帧率序列
    ...
    for f, dm in seq:
        for _ in range(max(1, round(dm * fps / 1000))):
            ...
            idx += 1
    ffmpeg(tmp, fps, os.path.join(OUT, "03-toggle.mp4"))
...
def vertical_frame(img, label, label_fill, fg=WHITE):
    ...
    center_text(d, cw // 2, 1750, "开源免费 · GitHub 搜 cursor-zh", font(30), GREEN)
    return c.convert("RGB")
...
def make_vertical():
    ...
    tmp = os.path.join(OUT, "_frames_v")
    os.makedirs(tmp, exist_ok=True)
    idx = 0
    for f, dm in seq:
        for _ in range(max(1, round(dm * fps / 1000))):
            f.save(os.path.join(tmp, f"f{idx:04d}.png"))
            idx += 1
    ffmpeg(tmp, fps, os.path.join(OUT, "04-vertical.mp4"))
...
def make_xhs_cover():
    ...
    sh_ = 800
    sw = int(before_side.width * sh_ / before_side.height)  # ≈ 242
    gap = 150
    x1 = (cw - (sw * 2 + gap)) // 2
    x2 = x1 + sw + gap
    y0 = 500
    ...
    center_text(d, cw // 2, ch - 100, "GitHub 搜  cursor-zh", font(40, True), GREEN)
