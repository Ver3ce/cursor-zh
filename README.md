...
3. 之后每次用桌面的「Cursor (中文)」启动 Cursor。窗口保持打开，不要关。更新词典不用开命令行：在这个窗口输入 `u` 回车，或双击程序旁边的「更新词典」。
...
词典约 1950 条精确词 + 150 条正则模板，随 Cursor 更新会出现新的英文文案。补词方式见下文"参与贡献"。已安装的用户用窗口里的 `u` 或「更新词典」即可拉取 main 上的最新词典，不必重下 exe。
